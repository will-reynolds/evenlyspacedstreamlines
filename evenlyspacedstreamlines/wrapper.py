...
def evenly_spaced_streamlines(
    ...
    *,
    ...
    if orientation.shape[1] == 2:
        orientation = np.column_stack((orientation, np.zeros(orientation.shape[0])))

    # check number of columns
    if vertices.shape[1] != 3 or triangles.shape[1] != 3 or orientation.shape[1] != 3:
        raise ValueError(
            "Arguments 'vertices', 'faces' and 'orient' must all have 3 columns."
        )

    # check array sizes
    if orientation.shape[0] != triangles.shape[0]:
        raise ValueError(
            "Arguments 'triangles' and 'orient' must have the same number of rows."
        )
